...
Простая математика, переменные, функции, классы и юзы и всякое другое
...
```
* extends -
  Классы можно расширить, добавив им функционала (дополнительные методы), а также перезаписывать их с помощью пометки ```@Ovr```
  ```Castiel
  public Class a {
    void fun a(){
  ...
  ```
* implements -
  Позволяет использовать методы класса, @Ovr тут тоже работает
  ```Castiel
  public Class a {
    void fun a(){
...
## Объекты
TODO: задокументировать in и for in
